...
import getAttributes, {featureinfotemplates, getContent} from './getattributes';
...
const Featureinfo = function Featureinfo(options = {}) {
  ...
  const selectionStyles =
      selectionStylesOptions ? Style.createGeometryStyle(selectionStylesOptions)
                             : Style.createEditStyle();
  let savedPin = savedPinOptions
                     ? maputils.createPointFeature(savedPinOptions, pinStyle)
                     : undefined;
  const savedFeature = savedPin || savedSelection || undefined;
  const uiOutput = 'infowindow' in options ? options.infowindow : 'overlay';

  /**
   * Dispatches a clearselectionevent. Should be emitted when window is closed
   * or cleared but not when featureinfo is closed as a result of tool change.
   */
  const dispatchClearSelection = function
  dispatchClearSelection() { component.dispatch('clearselection', null); };
  ...
  /**
   * Clears selection in all possible infowindows (overlay, sidebar and
   * infoWindow) and closes the windows
   * @param {any} supressEvent Set to true when closing as a result of tool
   *     change to supress sending clearselection event
   */
  const clear = function clear(supressEvent) {
    ...
    if (selectionManager) {
      // clearSelection will fire an cleared event, but we don't want our
      // handler to emit a clear event as we are the one closing, so we stop
      // listening for a while.
      selectionManager.un('cleared', onSelectionManagerClear);
      // This actually closes infowindow as infowindow closes automatically when
      // selection is empty.
  ...
  /**
   * Callback called from overlay and sidebar when they are closed by their
   * close buttons
   */
  function onInfoClosed() { clear(false); }
  ...
  const getTitle = function getTitle(item) {
    ...
    if (item.layer) {
      if (typeof item.layer === 'string') {
        // bcuz in getfeatureinfo -> getFeaturesFromRemote only name of the
        // layer is set on the object! (old version before using SelectedItems
        // class)
    ...
    // This is very strange: layer above is only a string, could not possibly
    // have method.
    if (layer) {
    ...
    if (featureinfoTitle) {
      ...
      if (!title) {
        if (item instanceof SelectedItem) {
          title = item.getLayer().get('title') ? item.getLayer().get('title')
                                               : item.getLayer().get('name');
        } else {
  ...
  const dispatchNewSelection = function dispatchNewSelection(item) {
    // Make sure it actually is a SelectedItem. At least Search can call
    // render() without creating proper selectedItems when search result is
    // remote.
    if (item instanceof SelectedItem) {
  ...
  const dispatchToggleFeatureEvent = function dispatchToggleFeatureEvent(
      currentItem) {
    const toggleFeatureinfo =
        new CustomEvent('toggleFeatureinfo',
                        {detail : {type : 'toggleFeatureinfo', currentItem}});
  ...
  // TODO: direct access to feature and layer should be converted to getFeature
  // and getLayer methods on currentItem Must take into consideration that
  // search can send an item that is not a SelecedItem
  const callback = function callback(evt) {
    ...
    if (currentItemIndex !== null) {
      ...
      selectionLayer.clearAndAdd(
          clone, selectionStyles[currentItem.feature.getGeometry().getType()]);
  ...
  const initCarousel = function initCarousel(id) {
    const {length} =
        Array.from(document.querySelectorAll('.o-identify-content'));
  ...
  const initImageCarousel = function initImageCarousel(id, oClass, carouselId,
    ...
    const {length} =
        Array.from(carousel[0].querySelectorAll('div.o-image-content > img'));
  ...
  function getSelectionLayer() { return selectionLayer.getFeatureLayer(); }
  ...
  function getSelection() {
    ...
    if (firstFeature) {
      ...
      selection.id = firstFeature.getId() != null ? firstFeature.getId()
                                                  : firstFeature.ol_uid;
      // FIXME: typeof layer can not be string, and if it is it would probably
      // not have a property called type that is set to WFS
      ...
      if (selection.type === 'WFS') {
        const idSuffix = selection.id.substring(
            selection.id.lastIndexOf('.') + 1, selection.id.length);
        selection.id =
            `${selectionLayer.getSourceLayer().get('name')}.${idSuffix}`;
      }
      if (selection.type !== 'WFS') {
        // FIXME: typeof layer can not be string
        const name = typeof selectionLayer.getSourceLayer() === 'string'
                         ? selectionLayer.getSourceLayer()
                         : selectionLayer.getSourceLayer().get('name');
  ...
  const getPin = function getPin() { return savedPin; };

  const getHitTolerance = function getHitTolerance() { return hitTolerance; };
  ...
  const addLinkListener = function addLinkListener(el) {
    ...
    if (el && !el.hasAttribute('onClickModal')) {
      el.addEventListener('click', (e) => {
        ...
        case 'modal-full': {
          modalStyle =
              'max-width:unset;width:98%;height:98%;resize:both;overflow:auto;display:flex;flex-flow:column;';
          break;
        }
        default: {
          modalStyle =
              'resize:both;overflow:auto;display:flex;flex-flow:column;';
          break;
        }
        }
        Modal({
          title : targ.title,
          content : `<iframe src="${
              targ.href}" class=""style="width:100%;height:99%"></iframe>`,
          target : viewer.getId(),
          style : modalStyle,
          newTabUrl : targ.href
        });
  ...
  /**
   * Creates temporary attributes on a feature in order for featureinfo to
   * display attributes from related tables and display attachments as links.
   * Recursively adds attributes to related features in order to support multi
   * level relations. In order to do so, attributes are also added to the
   * related features. The hoistedAttributes array can be used to remove all
   ...
   * @param {any} hoistedAttributes An existing array that is populated with the
   *     added attributes.
   */
  async function hoistRelatedAttributes(parentLayer, parentFeature,
                                        hoistedAttributes) {
    // This function is async and called recursively, DO NOT USE forEach!!! (It
    // won't work)

    let dirty = false;
    // Add attachments first but only if configured for attribute hoisting
    const attachmentsConf = parentLayer.get('attachments');
    if (attachmentsConf && attachmentsConf.groups.some(
                               g => g.linkAttribute || g.fileNameAttribute)) {
      ...
      for (let i = 0; i < ac.getGroups().length; i += 1) {
        ...
        if (currAttrib.linkAttribute) {
          ...
          hoistedAttributes.push(
              {feature : parentFeature, attrib : currAttrib.linkAttribute});
          dirty = true;
        }
        if (currAttrib.fileNameAttribute) {
          hoistedAttributes.push(
              {feature : parentFeature, attrib : currAttrib.fileNameAttribute});
        ...
        if (layerConfig.promoteAttribs) {
          ...
          for (let j = 0; j < layerConfig.promoteAttribs.length; j += 1) {
            ...
            childFeatures.forEach(child => {
              ...
              // Here one could imagine supporting more attribute types, but
              // html is pretty simple and powerful
              if (currAttribConf.html) {
                const val = replacer.replace(currAttribConf.html,
                                             child.getProperties());
                resarray.push(val);
              }
            });
            // Then actually aggregate them. Its a two step operation so in the
            // future we could support more aggregate functions, like min(),
            // max() etc and also to avoid appending manually and handle that
            // pesky separator on last element.
            const sep =
                currAttribConf.separator ? currAttribConf.separator : '';
            const resaggregate = resarray.join(sep);
            parentFeature.set(currAttribConf.parentName, resaggregate);
            hoistedAttributes.push(
                {feature : parentFeature, attrib : currAttribConf.parentName});
            dirty = true;
          }
        }
      }
    }
    // Only returns if top level is dirty. We don't build content for related
    // objects.
    return dirty;
  }
  ...
  async function addRelatedContent(item) {
    ...
    const updated = await hoistRelatedAttributes(
        item.getLayer(), item.getFeature(), hoistedAttributes);
    if (updated) {
      // Update content as the pseudo attributes have changed
      // Ideally this should have been made before SelectedItem was created, but
      // that changes so much in the code flow as the getAttachment is async-ish
      item.setContent(
          getAttributes(item.getFeature(), item.getLayer(), viewer.getMap()));
    }
    // Remove all temporary added attributes. They mess up saving edits as there
    // are no such fields in db.
    hoistedAttributes.forEach(
        hoist => { hoist.feature.unset(hoist.attrib, true); });
  }
  ...
  const doRender = function doRender(identifyItems, target, coordinate,
    ...
    content =
        '<div id="o-identify"><div id="o-identify-carousel" class="flex"></div></div>';
    switch (target) {
    case 'overlay': {
      popup = Popup(`#${viewer.getId()}`, {closeCb : onInfoClosed});
      popup.setContent({content, title : getTitle(items[0])});
      ...
      selectionLayer.clearAndAdd(clone, selectionStyles[geometry.getType()]);
      selectionLayer.setSourceLayer(items[0].layer);
      const coord = geometry.getType() === 'Point' ? geometry.getCoordinates()
                                                   : coordinate;
      carouselIds.forEach((carouselId) => {
        let targetElement;
        const elements =
            document.getElementsByClassName(`o-image-carousel${carouselId}`);
        Array.from(elements).forEach(element => {
          if (!element.closest('.glide__slide--clone')) {
            targetElement = element;
          }
        });
        const imageCarouselEl =
            document.getElementsByClassName(`o-image-carousel${carouselId}`);
        if (imageCarouselEl.length > 0) {
          initImageCarousel(`#o-image-carousel${carouselId}`,
                            `.o-image-content${carouselId}`, carouselId,
                            targetElement);
        }
      });
      ...
      if (!ignorePan) {
        overlayOptions.autoPan = {margin : 55, animation : {duration : 500}};
      }
      if (items[0].layer && items[0].layer.get('styleName')) {
        ...
        if (itemStyle && itemStyle[0] && itemStyle[0][0] &&
            itemStyle[0][0].overlayOptions) {
    ...
    case 'sidebar': {
      sidebar.setContent({content, title : getTitle(items[0])});
      const contentDiv = document.getElementById('o-identify-carousel');
      items.forEach((item) => {
        if (item.content instanceof Element) {
          contentDiv.appendChild(item.content);
    ...
    // Don't send event for infowindow. Infowindow will send an event that
    // triggers sending the event later.
    if (target === 'overlay' || target === 'sidebar') {
      dispatchToggleFeatureEvent(items[0]);
    }
  };
  /**
   * Renders the feature info window. Consider using showInfo instead if calling
   * using api.
   ...
   * @param {any} opts Additional options. Supported options are : ignorePan,
   *     disable auto pan to popup overlay.
   */
  const render = function render(identifyItems, target, coordinate, opts = {}) {
    ...
    identifyItems.forEach(currItem => {
      // At least search can call render without SelectedItem as Items, it just
      // sends an object with the least possible fields render uses so we need
      // to exclude those from related tables handling, as we know nothing about
      // them
      if (currItem instanceof SelectedItem && currItem.getLayer()) {
        // Fire off a bunch of promises that fetches attachments and related
        // tables
        requests.push(addRelatedContent(currItem));
      }
    });
    // Wait for all requests. If there are no attachments it just calls .then()
    // without waiting.
    Promise.all(requests)
        .catch((err) => {
          console.log(err);
          alert(
              'Kunde inte hämta relaterade objekt. En del fält från relaterade objekt kommer att vara tomma.');
        })
        .then(() => {
  ...
  /**
   * Shows the featureinfo popup/sidebar/infowindow for the provided features.
   * Only vector layers are supported.
   * @param {any} fidsbylayer An object containing layer names as keys with a
   *     list of feature ids for each layer
   * @param {any} opts An object containing options. Supported options are :
   *     coordinate, the coordinate where popup will be shown. If omitted first
   *     feature is used.
   *                                                                         ignorePan,
   * do not autopan if type is overlay. Pan should be supressed if view is
   * changed manually to avoid contradicting animations.
   * @returns nothing
   */
  const showInfo = function showInfo(fidsbylayer, opts = {}) {
    ...
    keys.forEach(layername => {
      fidsbylayer[layername].forEach(currFeatureId => {
        ...
        const newItem =
            getFeatureInfo.createSelectedItem(f, layer, map, grouplayers);
        newItems.push(newItem);
      });
    });
    render(newItems, identifyTarget,
           opts.coordinate ||
               maputils.getCenter(newItems[0].getFeature().getGeometry()),
           opts);
  };

  /**
   * Shows the featureinfo popup/sidebar/infowindow for the provided features
   * and fit the view to it.
   * @param {any} featureObj An object containing layerName and feature.
   *     "feature" is either one Feature or an Array of Feature
   * @param {any} opts An object containing options. Supported options are :
   *     coordinate, the coordinate where popup will be shown. If omitted first
   *     feature is used.
   *                                                                         ignorePan,
   * do not autopan if type is overlay. Pan should be supressed if view is
   * changed manually to avoid contradicting animations. maxZoomLevel, max level
   * for zooming on feature.
   * @returns nothing
   */
  const showFeatureInfo = function showFeatureInfo(featureObj, opts) {
    const thisOpts = {...{ignorePan : true}, ...opts};
    ...
    if (Array.isArray(featureObj.feature)) {
      featureObj.feature.forEach(feature => {
        const newItem =
            getFeatureInfo.createSelectedItem(feature, layer, map, grouplayers);
        newItems.push(newItem);
      });
    } else {
      const newItem = getFeatureInfo.createSelectedItem(
          featureObj.feature, layer, map, grouplayers);
      newItems.push(newItem);
    }
    if (newItems.length > 0) {
      render(newItems, identifyTarget,
             thisOpts.coordinate ||
                 maputils.getCenter(newItems[0].getFeature().getGeometry()),
             thisOpts);
      viewer.getMap().getView().fit(
          maputils.getExtent(newItems.map(i => i.getFeature())),
          {maxZoom : thisOpts.maxZoomLevel});
    }
  };

  const onClick = function onClick(evt) {
    savedPin = undefined;
    // Featurinfo in two steps. Concat serverside and clientside when serverside
    // is finished
    const pixel = evt.pixel;
    const map = viewer.getMap();
    const coordinate = evt.coordinate;
    const clientResult = getFeatureInfo.getFeaturesAtPixel(
        {coordinate, clusterFeatureinfoLevel, hitTolerance, map, pixel},
        viewer);
    // Abort if clientResult is false
    if (clientResult !== false) {
      getFeatureInfo.getFeaturesFromRemote({coordinate, map, pixel}, viewer)
          .then((data) => {
            ...
            if (result.length > 0) {
              ...
              render(result, identifyTarget, evt.coordinate);
                       ...
                       (identifyTarget === 'infowindow' &&
                        selectionManager.getNumberOfSelectedItems() > 0)) {
              clear(false);
            } else if (pinning) {
              const resolution = map.getView().getResolution();
              sidebar.setVisibility(false);
              setTimeout(() => {
                if (!maputils.checkZoomChange(resolution,
                                              map.getView().getResolution())) {
                  savedPin =
                      maputils.createPointFeature(evt.coordinate, pinStyle);
  ...
  return Component({
    ...
    onAdd(e) {
      ...
      if (selectionManager) {
        selectionManager.on('highlight',
                            evt => dispatchToggleFeatureEvent(evt));
        selectionManager.on('cleared', onSelectionManagerClear);
      }
      map.on(clickEvent, onClick);
      viewer.on('toggleClickInteraction', (detail) => {
        // This line of beauty makes feature info active if explicitly set
        // active of another control yields active state. which effectively
        // makes this the default tool.
        if ((detail.name === 'featureinfo' && detail.active) ||
            (detail.name !== 'featureinfo' && !detail.active)) {
          setActive(true);
        } else {
          setActive(false);
        }
      ...
      if (viewer.getViewerOptions().featureinfoOptions.changePointerOnHover) {
        ...
        document.addEventListener('enableInteraction', evt => {
          ...
          // Avoid getting stuck in pointer mode if user manages to enable
          // editing while having pointer over a clickable feature. If the user
          // manages to disable editing while standing on a clickable feature,
          // it will remain arrow until moved. (Sorry)
          map.getViewport().style.cursor = '';
        });

        // Check if there is a clickable feature when mouse is moved.
        map.on('pointermove', evt => {
          if (!pointerActive || evt.dragging)
            return;
          let cursor = '';
          const features = map.getFeaturesAtPixel(
              evt.pixel,
              {layerFilter(layer) { return layer.get('queryable'); }});
          if (features.length > 0) {
            ...
            layerGroups.forEach(
                item => item.getLayersArray().forEach(element => {
                  if (element.get('queryable') && element.get('visible')) {
                    layerArray.push(element);
                  }
                }));
            const layers = viewer.getQueryableLayers().filter(
                layer => layer instanceof BaseTileLayer || layer instanceof
                                                               ImageLayer);
            if (layers) {
              layers.forEach(element => layerArray.push(element));
            }
