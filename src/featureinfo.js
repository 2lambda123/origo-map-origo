...
const Featureinfo = function Featureinfo(options = {}) {
  ...
  const selectionStyles = selectionStylesOptions
    ? Style.createGeometryStyle(selectionStylesOptions)
    ...
    if (featureinfoTitle) {
      ...
      if (!title) {
        if (item instanceof SelectedItem) {
          title = item.getLayer().get("title")
            ? item.getLayer().get("title")
  ...
  const initCarousel = function initCarousel(id) {
    const { length } = Array.from(
      document.querySelectorAll(".o-identify-content"),
    );
  ...
  const initImageCarousel = function initImageCarousel(
    id,
    oClass,
    carouselId,
    targetElement,
    ...
    const { length } = Array.from(
      carousel[0].querySelectorAll("div.o-image-content > img"),
    );
   ...
   *     added attributes.
   */
  async function hoistRelatedAttributes(
    parentLayer,
    parentFeature,
    hoistedAttributes,
  ) {
    ...
    if (
      attachmentsConf &&
      attachmentsConf.groups.some((g) => g.linkAttribute || g.fileNameAttribute)
    ) {
      ...
      for (let i = 0; i < ac.getGroups().length; i += 1) {
        ...
        if (currAttrib.linkAttribute) {
          ...
          hoistedAttributes.push({
            ...
            attrib: currAttrib.linkAttribute,
          });
          dirty = true;
        }
        if (currAttrib.fileNameAttribute) {
          hoistedAttributes.push({
            feature: parentFeature,
            attrib: currAttrib.fileNameAttribute,
          });
    ...
    switch (target) {
      case "overlay": {
        ...
        carouselIds.forEach((carouselId) => {
          ...
          const imageCarouselEl = document.getElementsByClassName(
            `o-image-carousel${carouselId}`,
          );
          if (imageCarouselEl.length > 0) {
            initImageCarousel(
              `#o-image-carousel${carouselId}`,
              `.o-image-content${carouselId}`,
              carouselId,
              targetElement,
            );
          }
        ...
        const popupHeight =
          document.querySelector(".o-popup").offsetHeight + 10;
  ...
  const showFeatureInfo = function showFeatureInfo(featureObj, opts) {
    ...
    if (newItems.length > 0) {
      ...
      viewer
        .getMap()
        .getView()
        .fit(maputils.getExtent(newItems.map((i) => i.getFeature())), {
  ...
  const onClick = function onClick(evt) {
    ...
    if (clientResult !== false) {
      getFeatureInfo
        .getFeaturesFromRemote({ coordinate, map, pixel }, viewer)
        .then((data) => {
          ...
          if (result.length > 0) {
            ...
            selectionLayer.getFeatures().length > 0 ||
            (identifyTarget === "infowindow" &&
            ...
            setTimeout(() => {
              if (
                !maputils.checkZoomChange(
                  resolution,
                  map.getView().getResolution(),
                )
              ) {
                savedPin = maputils.createPointFeature(
                  evt.coordinate,
                  pinStyle,
                );
  ...
  return Component({
    ...
    onAdd(e) {
      ...
      if (selectionManager) {
        selectionManager.on("highlight", (evt) =>
          dispatchToggleFeatureEvent(evt),
        );
        selectionManager.on("cleared", onSelectionManagerClear);
      }
      map.on(clickEvent, onClick);
        ...
        map.on("pointermove", (evt) => {
          ...
          if (features.length > 0) {
            ...
            layerGroups.forEach((item) =>
              item.getLayersArray().forEach((element) => {
            ...
            const layers = viewer
              ...
              .filter(
                (layer) =>
                  layer instanceof BaseTileLayer || layer instanceof ImageLayer,
              );
            if (layers) {
