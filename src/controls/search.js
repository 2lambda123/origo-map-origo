...
const Search = function Search(options = {}) {
   ...
   * A map service is used to get the geometry and attributes. The layer is
   * defined as an ordinary layer in the layer config section. Option 2. Same as
   * option 1 but for single layer search. layerName is defined as an option and
   * is not included in the search response. In this case geometryAttribute and
   * layerName must be provided. Option 3. Complete feature info is included in
   * the search result. In this case titleAttribute, contentAttribute and
   * geometryAttribute must be provided. Option 4. This is a single table
   * search. No layer is defined. In this case geometryAttribute and title must
   * be defined. Option 5. Feature info is shown without selection in the map.
   * This is a simple single table search. In this case title, northing and
   * easting must be defined.
   */
  ...
  function selectHandler(evt) {
    ...
    if (layerNameAttribute && idAttribute) {
      ...
      id = data[idAttribute];
      getFeature(id, layer, source, projCode, proj).then((res) => {
        let featureWkt;
        let coordWkt;
        if (res.length > 0) {
  ...
  /*
  The label property in Awesomplete is used to store the feature id. This way
  the properties of each feature in the search response will be available in
  event handling. The complete properties are stored in a tempory db called
  searchDb. This is a workaround for a limit in Awesomplete that can only store
  data in the fields label and text. The data-category attribute is used to make
  a layer division in the sugguestion list.
  */
  ...
  function initAutocomplete() {
    ...
    function makeRequest(params) {
      const {reqHandler, obj, opt = {}, ignoreGroup = false, complete = false} =
          params;
